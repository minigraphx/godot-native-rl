...
- `examples/quadruped_walk` — 3D continuous-control **locomotion**: a code-built articulated quadruped (8 hinge-joint motors, Jolt physics) trained with PPO (`./scripts/train_quadruped.sh`). Ships a trained ncnn net that **walks forward** (~10 m before falling), deployed in `quadruped_walk_track.tscn` (camera + distance HUD), plus a learning-stage spread under `models/stages/` (500k/2M/5M steps) so you can watch the creature progress from flailing to walking. Behavioral forward-distance + golden-inference regressions ([#60](https://github.com/minigraphx/godot-native-rl/issues/60))
