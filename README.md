...
- `examples/gridworld` — **Unity GridWorld parity** + the `GridSensor2D` worked example: navigate an 8×8 grid to the goal, avoid pits (5 discrete actions, 5×5×2-layer grid-sensor obs + goal vector) (`./scripts/train_gridworld.sh`) ([#48](https://github.com/minigraphx/godot-native-rl/issues/48)). Also demonstrates **discrete action masking** ([#385](https://github.com/minigraphx/godot-native-rl/issues/385), Unity `WriteDiscreteActionMask` parity): the agent masks off-grid moves via `get_action_mask()`, applied with **native masked ncnn inference at deploy** (no ONNX/.NET runtime does per-step masking without custom glue); `MASKABLE=1 ./scripts/train_gridworld.sh` trains the sb3-contrib MaskablePPO variant — see [docs/guide/action-masking.md](docs/guide/action-masking.md)
- `examples/go_to_goal` — **goal-conditioned observations** ([#386](https://github.com/minigraphx/godot-native-rl/issues/386), Unity ML-Agents goal-signal parity): 3 targets, exactly one signaled per episode via a `GoalSensor` one-hot, and **one** policy reaches whichever is signaled (12-dim obs — 9 `RelativePositionSensor2D` + 3 goal one-hot — 5 discrete actions, trained with stock SB3 PPO since the goal is just obs; `./scripts/train_go_to_goal.sh`). The shipped net reaches all 3 distinct signaled targets (seed 42: 17 goals, by_id [7,5,5]); the **goal-blind ablation** proves the signal is load-bearing — the SAME net with its goal channel zeroed collapses to chance, failing the 2:1 goals:wrong bar (seed 42: 4 goals, one target, by_id [4,0,0]), and two always-on regressions assert the contrast (mirroring the [#378](https://github.com/minigraphx/godot-native-rl/issues/378) memory-chase ablation)
...
- `examples/chase_the_target/chase_memory.tscn` — **recurrent (LSTM) deploy, end-to-end** ([#378](https://github.com/minigraphx/godot-native-rl/issues/378)): the **memory chase** — the target's observation BLINKS out (visible 2 decisions, hidden 6), so the policy must *remember* where it was and keep homing while blind. Trained with sb3-contrib **RecurrentPPO** (`./scripts/train_chase_memory.sh`, opt-in `requirements-recurrent.txt`), the LSTM actor exported to a multi-IO ncnn graph + `.recurrent.json` sidecar (`scripts/export_recurrent_ppo.py`, carried-sequence parity-gated) and deployed through the native hidden-state carry. The shipped net catches 7–11/1800 frames; the regression pair proves the memory is **load-bearing**: the same net with its hidden state zeroed before every decision catches **0**
