...
- `examples/go_to_goal` — **goal-conditioned observations** ([#386](https://github.com/minigraphx/godot-native-rl/issues/386), Unity ML-Agents goal-signal parity): 3 targets, exactly one signaled per episode via a `GoalSensor` one-hot, and **one** policy reaches whichever is signaled (12-dim obs — 9 `RelativePositionSensor2D` + 3 goal one-hot — 5 discrete actions, trained with stock SB3 PPO since the goal is just obs; `./scripts/train_go_to_goal.sh`). The shipped net reaches all 3 distinct signaled targets (seed 42: 17 goals, by_id [7,5,5]); the **goal-blind ablation** proves the signal is load-bearing — the SAME net with its goal channel zeroed collapses to one target (4 goals, by_id [4,0,0]), and two always-on regressions assert the contrast (mirroring the [#378](https://github.com/minigraphx/godot-native-rl/issues/378) memory-chase ablation)
