...
- `examples/seek_target` — the **`RelativePositionSensor2D` worked example** ([#38](https://github.com/minigraphx/godot-native-rl/issues/38)): reach a goal while dodging a patrolling hazard, where the agent's ENTIRE observation is one drop-in sensor observing both (goal + hazard slots, 4 floats, zero hand-coded obs — `collect_sensors()` auto-discovery). The shipped net is the first example trained **entirely in-engine** by `ESTrainer` with sep-CMA-ES: no Python, no export step, the checkpoint is the deploy artifact
