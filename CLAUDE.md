...
- **Actionable backlog (pick up by number):** `docs/BACKLOG.md` — any session (incl. mobile) can
  ...
  - **Done:** 1 (Signal→Reward + RewardBuilder), 2 (export_to_ncnn helper), 3 (RaycastSensor2D/3D),
    ...
    20 (multi-policy `policy_name` wire field — `agent_policy_names` in env_info; *slice* of item
    20, the rest of the catalog line is still open; trained example is item 45).
    9 partial (socket
