...
- **Train (goal-conditioned reach, PPO discrete):** `./scripts/train_go_to_goal.sh` — stock SB3 PPO
  ...
  5 discrete actions; the goal is just obs, so no new trainer. Exports the deployed net to
  `examples/go_to_goal/models/go_to_goal.ncnn.*`. The **goal-blind ablation** proves the signal is
  load-bearing: the trained net reaches all 3 distinct signaled targets (17 goals / by_id [7,5,5]),
  the SAME net with its goal channel zeroed collapses to one target (4 goals / by_id [4,0,0]).
