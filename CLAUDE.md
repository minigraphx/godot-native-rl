...
`addons/godot_native_rl/` (`sync.gd`/`NcnnSync`, `controllers/`, `reward/`, `sensors/` (+ drop-in `scenes/`; incl. `EntitySensor2D/3D` — #46 M1 variable-length entity obs block: up to N nearest entities as `[N*F zero-padded][N presence flags]`, pure `EntityObsMath`, for the attention encoder; and `GoalSensor` — #386 dimension-agnostic `num_goals`-wide one-hot of the current goal (pure `GoalMath.one_hot`) for goal-conditioned policies),
...
**Architecture + data flow + deploy contract:
...
- **Build the extension (web/WASM):** `source ~/emsdk/emsdk_env.sh && scripts/cross/build_web.sh` (single-threaded; needs emsdk 3.1.64). No COOP/COEP headers required at deploy — see `docs/dev/building.md`. Model `*.ncnn.param`/`*.ncnn.bin` are auto-packed into exports by the enabled addon's `EditorExportPlugin` (`addons/godot_native_rl/export/`), EXCEPT under the skip roots from the `godot_native_rl/export/skip_roots` ProjectSetting (addon default: `res://test` only; THIS repo also prunes its scratch root `res://models` — downstream users' `res://models` is deploy content and stays packed, #337; pruned files are push_warning'd at export). Without the plugin, set an `include_filter` by hand. (Raw data files the Godot exporter skips otherwise — affects all platforms.)
...
- **Train (rover, resumable):** `./scripts/train_rover.sh` — checkpoints to `models/rover_checkpoints/`
  ...
  run manifest; the deploy-policy exporters prefer it, and since #146 the trainers' **inline
  end-of-run export ships the blessed best too** — `chase`/`fly_by`/`ball_chase` load+export it via
  `checkpoints.deploy_export_checkpoint` instead of the final in-memory model; **all four SB3
  trainers** — chase, rover, ball_chase, fly_by — accept `BEST_CHECKPOINT=1`). On macOS/Apple Silicon wrap
...
- **Train (GridWorld, PPO discrete):** `./scripts/train_gridworld.sh` — Unity-parity grid navigation
  ...
  goal vector), `ParallelArena2D` 8 worlds, 300k steps, ONNX → `export_to_ncnn.py`. **`MASKABLE=1
  ./scripts/train_gridworld.sh`** (#385) trains the sb3-contrib **MaskablePPO** masked variant over
  `MaskableStableBaselinesGodotEnv` (opt-in dep — the same sb3-contrib as RecurrentPPO); it exports
  the raw-logits head, and the discrete action mask is re-applied game-side at deploy (`ActionDecode`,
  `-6e4` sentinel) so train and deploy agree. Guarded-smoked in `run_tests.sh`.
- **Train (goal-conditioned reach, PPO discrete):** `./scripts/train_go_to_goal.sh` — stock SB3 PPO
  over the GoToGoal env (#386): 3 targets, exactly one signaled per episode via a `GoalSensor` one-hot,
  ONE net reaches whichever is signaled. 12-dim obs (9 `RelativePositionSensor2D` + 3 goal one-hot),
  5 discrete actions; the goal is just obs, so no new trainer. Writes `models/go_to_goal.{zip,onnx}`;
  convert with `scripts/export_to_ncnn.py models/go_to_goal.onnx --outdir examples/go_to_goal/models`
  (the shipped net is `examples/go_to_goal/models/go_to_goal.ncnn.*`). The **goal-blind ablation** proves
  the signal is load-bearing: the trained net reaches all 3 distinct signaled targets (seed 42: 17 goals
  / by_id [7,5,5]), the SAME net with its goal channel zeroed collapses to chance — failing the 2:1
  goals:wrong bar (seed 42: 4 goals / by_id [4,0,0], one target).
...
- **Train (sorter, attention encoder — CleanRL PPO):** `./scripts/train_sorter.sh` — variable-length
  entity-obs PPO (#46/#258 M2): the shared trunk is `scripts/attention_encoder.py` (masked
  multi-head self-attention over the `EntitySensor2D` `[6*4 entities][6 flags]` block → masked
  mean-pool), which reshapes the flat obs internally so the CleanRL loop is unchanged. Defaults to
  the 8-world `sorter_train_parallel.tscn`. **Export is the DIRECT hand-written `attention_policy_layers`
  writer, NOT ONNX/pnnx** — the spike (`scripts/spike_attention_ncnn.py`) recorded `pnnx-decomposed`
  (pnnx can't fuse a hand-built MHA), and the direct path is byte-pinned to the #307 ncnn-verified
  fixtures (torch↔ncnn parity ~7e-5, argmax exact). `TIMESTEPS`/`NUM_STEPS` (lower `NUM_STEPS` for a
  fast smoke so a small `TIMESTEPS` still yields ≥1 update under 8 envs)/`SPEEDUP`/`ACTION_REPEAT`/
  `OUTDIR`/`STEM`/`SCENE` overrides. The torch encoder is numerically identical to the ncnn replica
  (`make_synthetic_attention.py::encoder_forward`) — the fp16-safe `-6e4` mask and per-head `1/√d`
  scale are load-bearing (train/deploy diverge otherwise).
- **Train (sorter, SB3 attention — PPO):** `./scripts/train_sorter_sb3.sh` — the #259 M3 SB3 parity
  path: stock SB3 PPO over `StableBaselinesGodotEnv` with `policy_kwargs` =
  `AttentionFeaturesExtractor` (wraps the SAME `attention_encoder.py`) + **`net_arch=[]`** (so
  `action_net` is a single linear head → the deploy actor is `encoder → head`, the identical M2
  export contract). Exports via the same direct `attention_policy_layers` writer (pulls
  `policy.features_extractor.encoder` + `policy.action_net`), producing a same-shape ncnn graph as
  the CleanRL trainer. No VecNormalize (raw obs = training input). `TIMESTEPS`/`NUM_STEPS`/`SPEEDUP`/
  `ACTION_REPEAT`/`OUTDIR`/`STEM`/`SCENE` overrides. A deterministic SB3→ncnn export-parity unit test
  (`test/python/test_sb3_attention_export_parity.py`, no Godot) is the CI gate.
