...
| Native in-engine ES training (`ESTrainer`, #131) | No Python/socket/backprop; OpenAI-ES or sep-CMA-ES (diagonal covariance, O(θ)/gen; measured 13.4 vs 2.3 mean fitness on chase at 200 gens) over the ncnn forward pass, CRN seeding + k-episode fitness, checkpoints are deploy-ready `.ncnn` pairs, warm-start on-device fine-tuning |
