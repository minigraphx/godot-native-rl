...
Goal-conditioned reach: the agent is signaled a target and must navigate to it (the seek
task + a goal channel), so ONE net serves MANY goals. Run FIRST (server on 11008), then
launch the Godot scene. See scripts/train_go_to_goal.sh. Discrete/continuous MLP ->
the chase-standard ONNX export path -> export_to_ncnn.py.
